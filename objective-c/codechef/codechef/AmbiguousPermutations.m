...
+ (NSString *)main:(NSInteger)n withInput:(NSString *)input {
    assert(n >= 1 && n <= 100000);
    NSArray *permutation = [input componentsSeparatedByString:@" "];
    NSArray *inversedPermutation = [self createInversePermutation:permutation];
    return [inversedPermutation isEqualToArray:permutation] ? @"ambiguous" : @"unambiguous";
}
